...
import { FiBell, FiLogOut, FiMenu, FiMoon, FiSettings, FiShoppingCart, FiSun, FiThumbsUp } from 'react-icons/fi'
...
const Header = ({ session, isAdmin, isCustomer, onSidebarOpen }) => {
	...
	return (
		<chakra.header bg="#FFFFFFBF" backdropFilter="blur(12px)" position="sticky" top={0} zIndex={99} _dark={{ bg: '#14161FBF' }}>
			<Flex align="center" gap={6} mx="auto" px={6} h={16} w="full" maxW={isAdmin ? 1536 : 1280}>
				<Flex flex={1} justify="start" align="center">
					<NextLink href="/" passHref>
						<Flex align="center" gap={3}>
							<Romadan h={32} w={24} />

							<Text display={{ base: 'none', md: 'flex' }} fontSize="2xl" fontWeight="semibold" lineHeight={8} color="accent-1">
								Romadan
							</Text>
						</Flex>
					</NextLink>
					...
					<Flex display={{ base: 'none', lg: 'flex' }} flex={3} justify="center" align="center">
						<Flex align="center" gap={8}>
							<NextLink href="/products" passHref>
								<Link as="span" active={router.pathname.includes('products') ? 1 : 0}>
									Products
								</Link>
							</NextLink>
				...
				<Flex flex={1} justify="end" align="center">
					<Flex align="center" gap={3}>
						<Flex align="center">
							<IconButton variant="ghost" colorScheme="brand" icon={<FiShoppingCart size={16} fill="currentColor" />} />
							<IconButton variant="ghost" colorScheme="brand" icon={<FiBell size={16} fill="currentColor" />} />
						...
						{session ? (
							<>
								<Menu>
									<MenuButton display={{ base: 'none', lg: 'block' }}>
										<Avatar name={session.user.name} src={session.user.image} />
									</MenuButton>

									<MenuList display={{ base: 'none', lg: 'block' }} w={256}>
										<MenuItem>
								...
								<chakra.button display={{ base: 'block', lg: 'none' }} onClick={onSidebarOpen}>
									<Avatar name={session.user.name} src={session.user.image} />
								</chakra.button>
							</>
						) : (
							<>
								<IconButton display={{ base: 'flex', lg: 'none' }} variant="plain" size="xs" icon={<FiMenu size={24} />} onClick={onSidebarOpen} />
