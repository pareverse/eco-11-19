...
import { Avatar, Button, chakra, Flex, Grid, GridItem, Icon, Link, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Text } from '@chakra-ui/react'
import { FiEdit2, FiGrid, FiHeart, FiLogOut, FiMapPin, FiMessageSquare, FiPackage, FiPieChart, FiSettings, FiShoppingCart, FiThumbsUp, FiTrash2, FiUser, FiUsers } from 'react-icons/fi'
...
const Sidebar = ({ session, isAdmin, isCustomer, isUserPage, isSidebarOpen, onSidebarClose }) => {
	...
	return (
		<>
			...
			<chakra.aside bg="system" position="fixed" top={{ base: 0, lg: isAdmin ? 'auto' : isUserPage ? 'auto' : 0 }} left={{ base: isSidebarOpen ? 0 : -256, lg: isAdmin ? 'auto' : isSidebarOpen ? 0 : isUserPage ? 'auto' : -256 }} h={{ base: 'full', lg: isAdmin ? 'calc(100% - 64px)' : isUserPage ? 'calc(100% - 64px)' : 'full' }} w={256} transition="0.4s ease-in-out" zIndex={100}>
				<Grid templateRows={{ base: '64px 1fr auto', lg: isAdmin ? '1fr auto' : isUserPage ? '1fr' : '64px 1fr auto' }} h="full">
					<GridItem display={{ base: 'block', lg: isAdmin ? 'none' : isUserPage ? 'none' : 'block' }} my="auto" px="6">
						...
						{isAdmin ? (
							...
							<Flex direction="column" gap={1}>
								<NextLink href="/user/profile" passHref>
									<Link as="span" display="block" py={2} lineHeight={6} active={router.pathname.includes('profile') ? 1 : 0} onClick={onSidebarClose}>
										<Flex align="center" gap={3}>
								...
								<NextLink href="/user/address" passHref>
									<Link as="span" display="block" py={2} lineHeight={6} active={router.pathname.includes('address') ? 1 : 0} onClick={onSidebarClose}>
										<Flex align="center" gap={3}>
					...
					<GridItem display={{ base: 'block', lg: isUserPage ? 'none' : 'block' }} p={6}>
						{session ? (
							<Menu>
								<MenuButton>
									<Flex align="center" gap={3}>
								...
								<MenuList w={256}>
									...
									<MenuItem icon={<FiThumbsUp size={16} />}>Settings</MenuItem>
