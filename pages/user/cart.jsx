...
const Cart = () => {
	...
	const removeCartItemMutation = useMutation((data) => api.remove('/carts', data), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				render: () => <Toast title="Success" description="Cart item successfully removed." />
			})
		}
	})
					...
					<Card key={product.cart.id}>
						<Flex gap={6}>
							...
							<Flex justify="space-between" align="end" direction="column" gap={6}>
								...
								<Text fontSize="sm" fontWeight="medium" color="brand.default" cursor="pointer" onClick={() => removeCartItem(product.cart.id)}>
									Remove
								</Text>
							</Flex>
						</Flex>
